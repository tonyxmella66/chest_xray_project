...
def create_resnet50_model(num_classes=2):
    """Create a ResNet-50 model for binary classification"""
...
def train_single_fold(fold_num, target_feature, data_root, cv_splits_dir, 
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_filename = os.path.join(output_dir, 
                f'best_resnet50_fold{fold_num}_{target_feature.lower().replace(" ", "_")}.pth')
            torch.save({
                'epoch': epoch,
                'fold': fold_num,
    ...
    plt.title(f'Fold {fold_num}: Training and Validation Loss (ResNet-50)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    ...
    plt.title(f'Fold {fold_num}: Training and Validation Accuracy (ResNet-50)')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.legend()
...
def main(target_feature="Pleural Effusion", folds=[1, 2, 3, 4, 5], data_root="data",
         ...
         num_epochs=20, batch_size=32, learning_rate=0.001, output_dir="cv_outputs_resnet50", log_dir="."):
    """
    Main training function with cross-validation using ResNet-50
    ...
    """
    # Setup logging
    log_file, log_writer = setup_logging(log_dir, 'resnet50_training')
    ...
    tracker = EmissionsTracker(
        ...
        output_dir=log_dir,
        output_file=f'resnet50_training_emissions_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv',
        log_level='warning'
    )
    ...
    print(f"\n{'='*70}")
    print(f"Cross-Validation Summary for {target_feature} (ResNet-50)")
    print(f"{'='*70}")
    ...
    if os.path.exists(test_csv):
        ...
        test_tracker = EmissionsTracker(
            project_name="ResNet50_Test_Evaluation",
            output_dir=log_dir,
            output_file=f'resnet50_test_emissions_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv',
            log_level='warning'
        )
    ...
    fig.suptitle(f'Cross-Validation Results for {target_feature} (ResNet-50)', fontsize=16)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train ResNet-50 with Cross-Validation for Chest X-Ray Classification')
    ...
    parser.add_argument('--output_dir', type=str, default='cv_outputs_resnet50',
                       help='Directory to save all outputs (default: "cv_outputs_resnet50")')
    ...
    print(f"\n{'='*70}")
    print(f"Cross-Validation Training Configuration (ResNet-50)")
    print(f"{'='*70}")
