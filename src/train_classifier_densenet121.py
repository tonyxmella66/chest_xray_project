    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_filename = os.path.join(output_dir, 
                f'best_densenet_fold{fold_num}_{target_feature.lower().replace(" ", "_")}.pth')
            torch.save({
                'epoch': epoch,
                'fold': fold_num,
...
def main(target_feature="Pleural Effusion", folds=[1, 2, 3, 4, 5], data_root="data",
         ...
         num_epochs=20, batch_size=32, learning_rate=0.001, output_dir="cv_outputs", log_dir="."):
    """
    ...
    emissions_log = os.path.join(log_dir, f'densenet121_emissions_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    ...
    print(f"\n{'='*70}")
    print(f"Cross-Validation Summary for {target_feature}")
    print(f"{'='*70}")
    ...
    fig.suptitle(f'Cross-Validation Results for {target_feature}', fontsize=16)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train DenseNet with Cross-Validation for Chest X-Ray Classification')
    ...
    parser.add_argument('--output_dir', type=str, default='cv_outputs',
                       help='Directory to save all outputs (default: "cv_outputs")')
    ...
    print(f"\n{'='*70}")
    print(f"Cross-Validation Training Configuration")
    print(f"{'='*70}")
